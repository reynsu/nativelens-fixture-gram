...
**hydration phase**:
The interval between app boot and the moment `useSession.hydrate()` resolves the persisted session from AsyncStorage. The entry route at `app/index.tsx` reads `isLoggedIn` synchronously while hydration is still pending, so where the user is sent during the hydration phase depends on whether the route waits for `isHydrated`. The current implementation is a deliberate plant; do not "fix" it without updating the answer-key.
